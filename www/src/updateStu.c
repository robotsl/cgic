...
void printhead(void )
{
	...
	fprintf(cgiOut,"\n");
	fprintf(cgiOut,"<html><head><title>UpdateStu</title></head>");
	fprintf(cgiOut,"<body>");
}
...
int cgiMain()
{
	...
	fprintf(cgiOut,"<form ACTION=\"./sloveUpdateStu.cgi\" method=\"GET\">");
	...
	if(ret == SQLITE_OK){
		...
		fprintf(cgiOut,"name:<INPUT TYPE=\"text\" NAME=\"name\" SIZE=\"10\" value =\"%s\" required=\"required\"><BR>",dbresult[index++]);
		fprintf(cgiOut,"age:<INPUT TYPE=\"text\" NAME=\"age\" SIZE=\"10\" value =\"%s\" required=\"required\"><BR>",dbresult[index++]);
		fprintf(cgiOut,"class:<INPUT TYPE=\"text\" NAME=\"classes\" SIZE=\"10\" value =\"%s\" required=\"required\"><BR>",dbresult[index++]);
		fprintf(cgiOut,"major:<INPUT TYPE=\"text\" NAME=\"major\" SIZE=\"10\" value =\"%s\" required=\"required\"><BR>",dbresult[index++]);
