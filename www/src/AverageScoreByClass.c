...
int cgiMain()
{
	...
	ret = sqlite3_get_table(db,"select distinct classes from student group  by classes",&dbresult,&nrow,&ncolumn,&errmsg);
	if(ret == SQLITE_OK){
		index=ncolumn;
		for(i=0;i<nrow;i++){
			printf("<tr>");
			...
			for(j=0;j<ncolumn;j++){
				sprintf(sql,"select scores from score where sno =  (select sno  from student where classes = \'%s\');",dbresult[index]);
